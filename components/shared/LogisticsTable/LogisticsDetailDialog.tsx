...
import { useUpdateOrderShipping } from "@/lib/queries/useUpdateOrderShipping";
...
export function LogisticsDetailDialog({ orderId, open, onOpenChange }: Props) {
  const t = useTranslations("orders.detail");
  const tLogistics = useTranslations("logistics.detail");
  ...
  const { mutateAsync, isPending } = useUpdateOrderShipping(orderId ?? "");

  const [carrier, setCarrier] = useState("");
  const [trackingNumber, setTrackingNumber] = useState("");
  const [isEditing, setIsEditing] = useState(false);

  const isShippingSaved = !!(data?.trackingNumber?.trim());
  const fieldsDisabled = isPending || (isShippingSaved && !isEditing);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success(tLogistics("saveSuccess"));
      setIsEditing(false);
    } catch (err) {
      const message = err instanceof Error ? err.message : "";
      toast.error(message || tLogistics("saveError"));
    }
  };
            ...
            <div className="flex flex-col gap-3 rounded-lg border bg-muted/20 p-4">
              <div className="grid gap-3 sm:grid-cols-2">
                <div className="flex flex-col gap-2">
                  <Label htmlFor="logistics-carrier">{tLogistics("carrier")}</Label>
                  <Input
                    id="logistics-carrier"
                    value={carrier}
                    onChange={(e) => setCarrier(e.target.value)}
                    placeholder={tLogistics("carrierPlaceholder")}
                    disabled={fieldsDisabled}
                  />
                </div>
                <div className="flex flex-col gap-2">
                  <Label htmlFor="logistics-tracking">
                    {tLogistics("trackingNumber")}
                  </Label>
                  <Input
                    id="logistics-tracking"
                    value={trackingNumber}
                    onChange={(e) => setTrackingNumber(e.target.value)}
                    placeholder={tLogistics("trackingPlaceholder")}
                    disabled={fieldsDisabled}
                  />
                </div>
              </div>
              <div className="flex justify-end gap-2">
                {isShippingSaved && !isEditing && (
                  <Button
                    type="button"
                    size="sm"
                    variant="outline"
                    onClick={() => setIsEditing(true)}
                    aria-label={tLogistics("edit")}
                  >
                    <Pencil size={16} />
                  </Button>
                )}
                {(!isShippingSaved || isEditing) && (
                  ...
                  >
                    {isPending ? tLogistics("saving") : tLogistics("save")}
                  </Button>
                )}
              </div>
            </div>
